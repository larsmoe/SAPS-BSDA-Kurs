...
library(yaml)
setwd("C:/Users/XXX")
ps.name = "PS_1"; sol.file = paste0(ps.name,"_sol.Rmd")
libs = c("igraph", "yaml") # character vector of all packages you load in the problem set
...
#< award "Guter Start!"
Toll! Sie haben die ersten Basics zu Graphen in R verstanden und können Sie anwenden.  
Weiter so!
#>
...
#< award "Flexibler Netzwerker!"
Inzwischen kennen Sie drei verschiedene Arten, Graphen mit 'igraph' zu erzeugen:  
* manuell 
* aus einer Adjazenzmatrix 
* aus einem Datensatz

Das macht Sie zu einem "flexiblen Netzwerker"! Weiter so!
#>
...
```{r "2 d) 1", fig.height=14, fig.width=14}
#< task
...
- - -
```{r "3 a) 5", fig.height=10, fig.width=10}
#< task
...
- - -
#< quiz "Zusammenhängende Graphen"
parts:
 - question: Welche(r) der beiden ungerichteten Graphen ist (sind) zusammenhängend?
   choices:
      - graph_1*
      - graph_2
   multiple: TRUE
   success: Korrekt! Die beiden Graphen unterscheiden sich in der Kante von Knoten 3 zu Knoten 5. Wenn diese Brücke fehlt, ist der Graph nicht mehr zusammenhängend.
   failure: Leider nicht korrekt... Überlegen Sie noch einmal...
 - question: Welche(r) der drei gerichteten Graphen ist (sind) (mindestens) schwach zusammenhängend?
   choices:
        - graph_3*
        - graph_4
        - graph_5*
   ...
   success: Korrekt!
   failure: Leider nicht korrekt... Überlegen Sie noch einmal...
 - question: Welche(r) der drei gerichteten Graphen ist (sind) stark zusammenhängend?
   choices:
        - graph_3
        - graph_4
        - graph_5*
...
#>
#< award "Zusammenhangsmeister"
Herzlichen Glückwunsch! Bei zusammenhängenden und nicht zusammenhängenden Graphen kann Ihnen so schnell keiner etwas vormachen!  
Damit sind Sie bestens vorbereitet, die passenden R-Funktionen anzuwenden!
#>
- - -
...
#< award "Netzwerken für Fortgeschrittene"
Das klassische gerichtete oder ungerichtete Netzwerk ist längst nicht alles, 
was Sie bisher kennen...  
Gewichtete Netzwerke mit beliebigen Attributen 
und auch nicht zusammenhängende Netzwerke mit mehreren Komponenten sind 
Ihnen jetzt vertraut. Klasse!
#>
...
#< info "Rechnen mit Matrizen und Vektoren in R"
Seien *A* und *B* Matrizen in R. Dann erfolgen die Grundoperationen wie folgt:  
* Summe aus *A* und *B*: *A + B*  
* Differenz aus *A* und *B*: *A - B*  
* Matrizenmultiplikation: *A %***% B* 
* Elementweise Multiplikation von *A* und *B*: *A***B* 
* Transponierte von *A*: *t(A)* 
* Inverse von *A*: *solve(A)*

Numerische Vektoren werden behandelt wie Matrizen. Seien *v* und *w* Vektoren. Dann gilt speziell: 
* Skalarprodukt aus *v* und *w*: *v***t(w)*

Um die Summe aller Elemente eines Vektors oder einer Matrix *x* zu erhalten, benötigt man den Befehl *sum(x)*.
#>
...
#< award "Gratulation"
Toll! Sie haben das gesamte erste Problem Set gemeistert!  
Insgesamt sind Sie jetzt bezüglich der Inhalte im Skript auch in der Lage,  
R zur Lösung der Probleme und Anwendung der Methoden zu nutzen.
#>
