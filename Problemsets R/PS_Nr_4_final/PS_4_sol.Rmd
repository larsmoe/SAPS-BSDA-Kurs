...
library(yaml)
setwd("C:/Users/XXX")
ps.name = "PS_4"; sol.file = paste0(ps.name,"_sol.Rmd")
...
#>
karate_cl_edgebtw <- cluster_edge_betweenness(graph=karate, weights = NULL)
```
...
* *length(x)* gibt die Anzahl der Communities im Graphen an 
...
* *plot(x, y)* stellt die Communities grafisch dar, indem er den Graphen mit farbig markierten Communities plottet, x enthält das Community-Objekt und y das Graph-Objekt 
* *dendPlot(x)* stellt das hierarchische Cluster in einem Dendrogramm dar (Näheres dazu später), x enthält hierbei das Community-Objekt
...
- - -
```{r "7 a) 2", fig.height=10}
#< task
#Aus wie vielen Communities besteht karate?
#>
length(x=karate_cl_edgebtw)
#< task
#Wie groß sind die einzelnen Communities?
#>
sizes(communities=karate_cl_edgebtw)
#< task
...
#>
head(membership(communities=karate_cl_edgebtw))
#< task
...
#>
karate_cl_labelprop <- cluster_label_prop(graph=karate, weights=NA)
```
...
- - -
```{r "7 b) 2", fig.height=10}
#< task
#Plotten Sie den Graphen mit hervorgehobenen Communities
#>
plot(x=karate_cl_labelprop, y=karate)
```
...
#>
plot(x=cluster_label_prop(graph=karate, weights=NA), y=karate)
#< task
...
#>
karate_cl_louvain <- cluster_louvain(graph=karate, weights=NA)
```
...
- - -
```{r "7 c) 2", fig.height=10}
#< task
#Plotten Sie den Graphen mit hervorgehobenen Communities
#>
plot(x=karate_cl_louvain, y=karate)
...
#< award "Community Detector"
Communities sind nicht immer so leicht ausfindig zu machen - 
und vor allem nicht so eindeutig - aber sehr wertvoll ... 
Sie kennen nun Werkzeuge, um Communities zu finden. Das kann 
in der Praxis sehr hilfreich sein!
#>
...
- - -
```{r "8 a) 2", fig.height=10, fig.width=12}
#< task
#Erstellen Sie einen gerichteten Graphen aus dem Data Frame data_diff
#und speichern Sie ihn in der Variable g_diff
#>
g_diff <- graph_from_data_frame(d=data_diff, directed=TRUE, vertices = data_diff_nodes)
#< task
#Plotten Sie den Graphen g_diff wie oben beschrieben
#>
plot(x=g_diff, edge.label=E(g_diff)$Information, edge.arrow.size=1)
```

#< quiz "Interaktionsgraph"
parts:
 - question: Um was für einen Graphen handelt es sich beim Graphen g_diff?
   choices:
      - Interaktionsgraph*
      - Diffusionsgraph
   multiple: TRUE
...
- - -
```{r "8 a) 3", fig.height=10, fig.width=12}
#< task
#Erstellen Sie den Subgraphen zur Information A und
#speichern Sie ihn in der Variable g_diff_A
#>
g_diff_A <- subgraph.edges(graph=g_diff, 
                          eids = which(E(g_diff)$Information=="A"))
#< task
#Plotten Sie den Subgraphen g_diff_A mit denselben
#Argumenten wie im vorherigen Code-Chunk
#>
plot(x=g_diff_A, edge.label=E(g_diff)$Information, edge.arrow.size=1)
```
#< quiz "Diffusionsgraph"
parts:
 - question: Um was für einen Graphen handelt es sich beim Graphen g_diff_A?
   choices:
      - Interaktionsgraph
      - Diffusionsgraph*
   multiple: TRUE
...
#>
g_diff_A_indegree <- degree(graph=g_diff_A, mode="in")
#< task
#Ermitteln Sie diejenigen Knotenindizes, für die g_diff_A_indegree 0 ist
#>
which(g_diff_A_indegree==0)
```
...
#>
dist_diff_A_1 <- distances(graph=g_diff_A, v="k1", mode="out")
#< task
...
#>
sir_ba <- sir(graph=g_ba, beta=0.5, gamma=1.0)
#< task
#Plotten Sie die Variable sim_ba
#>
plot(x=sir_ba)
```
...
#< quiz "SIR Modell"
parts:
 - question: Je größer beta wird, desto
   choices:
      - flacher wird die Kurve
      - steiler steigt und fällt die Kurve und desto höher liegt das Maximum*
   multiple: TRUE
   success: Korrekt!
   failure: Leider nicht korrekt... Überlegen Sie noch einmal...
...
#>
sir_ba_1 <- sir(graph=g_ba, beta=1.0, gamma=1.0)
#< task
#Speichern Sie in der Variable sir_ba_5 die Simulation für beta=5.0
#>
sir_ba_5 <- sir(graph=g_ba, beta=5.0, gamma=1.0)
```

Der folgende Code-Chunk plottet die Mediane aller drei Simulationen, um die Simulationen miteinander vergleichen zu können. Die einzelnen Schritte können Sie den Kommentaren im Code entnehmen. Auf diese soll hier nicht detaillierter eingegangen werden. Falls Sie dennoch an einzelnen verwendeten Befehlen interessiert sind, können Sie die entsprechenden Beschreibungen in den R-Hilfeseiten finden. Betrachten Sie den Code und führen Sie ihn anschließend aus.
```{r "8 b) 5"}
#< task
...
plot(x=time_bins(x=sir_ba), y=median(x=sir_ba)[["NI"]],
     ...
     xlab="Time", ylab=expression(N[I](t)), main = "Ansteckungsrate beta = 0.5")
...
Abschließend haben Sie nun sogar in einen Bereich hineingeschnuppert, 
in dem R viel Potential besitzt: Simulationen. 
